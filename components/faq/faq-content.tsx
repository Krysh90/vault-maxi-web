...
export function FaqContent(): JSX.Element {
  const { entries, filter } = useFaqContext()
  return (
    <>
  ...
  return (
    <button onClick={() => setIsOpen(!isOpen)} className="w-full">
      <div className={`bg-light rounded-lg ${isOpen ? '' : 'md:h-12'} w-full px-2 py-1`}>
        ...
        {isOpen && (
          <div className="bg-dark flex flex-grow-0 flex-shrink-0 whitespace-pre-line items-start text-sm rounded-b-lg border-t border-light p-2 text-start ml-[2.375rem]">
            {faq.answer}
          </div>
        )}
      </div>
